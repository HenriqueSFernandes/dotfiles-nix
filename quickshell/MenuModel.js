/* exported parseMenuJsonc, mergeMenuSources, mergeAppRows, resolveRoute, matchesQuery, searchScore, displayRow, slugify */
...
function parseMenuJsonc(raw) {
  ...
  try {
    ...
    console.warn("Failed to parse menu jsonc:", e);
    return [];
  }
...
function mergeMenuSources(defaultItems, userItems) {
  ...
  for (var k3 = 0; k3 < nextOrder.length; k3++)
    nextItems[nextOrder[k3]].order = k3;
...
function displayRow(items, itemOrder, entry, detail, score, section) {
  ...
  return {
    ...
    childCount:
      entry.kind === "menu" || entry.kind === "link"
        ? childCount(items, itemOrder, target)
        : 0,
    action: entry.action || "",
...
// Node export for tests; QML has no `module`, so this is skipped there.
